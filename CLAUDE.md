...
internal/
  ...
  browserchaos/                  Browser chaos engine (headless Chrome disruption — network idle stall, SW poisoning, memory bombs, CSS bombs, iframe amplification)
...
- **Browser chaos engine** (`internal/browserchaos/`) targets headless Chrome crawlers (Puppeteer, Playwright, CF Browser Rendering). 4 severity levels: L1=network idle stall (fetch every 200-450ms prevents `networkidle2`), L2=+ServiceWorker poisoning+memory bombs (IndexedDB/Blob leaks)+hidden iframe amplification, L3=+CSS rendering bombs (nested calc, stacked blur filters, massive grids)+link flood (hundreds of invisible links), L4=+WASM CPU bomb+resource exhaustion (WebGL/AudioContext/Cache API)+EventSource keepalive. SW served at `/_bc/sw.js` with real URL (blob URLs blocked by headless Chrome). Iframe amplifier creates recursive chaos amplification (each iframe loads a page that also gets chaos injected). Controlled via `browser_chaos` feature flag and `browser_chaos_enabled`/`browser_chaos_level` admin config. Nightmare mode sets level 4. Tested against Cloudflare Browser Rendering API — confirmed 9x request amplification per page, all attack vectors injecting successfully.
